// src/components/templates/SuperAdminLetter.tsx
...
import { createLetter } from '../../store/slices/documentSlice';
...
import { generateAdminLetterDocx } from '../../utils/generateAdminLetterDocx';
import { generateAdminLetterPdf } from '../../utils/generateAdminLetterPdf';
...
interface SendLetterModalProps {
  ...
  loading: boolean;
  letterData: AdminLetterData;
  isGeneratingPDF: boolean;
}

const SendLetterModal: React.FC<SendLetterModalProps> = ({
  isOpen,
  onClose,
  onSend,
  users,
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl">
        ...
        <div className="mb-4 p-3 bg-stone-50 rounded-lg border border-stone-200">
          <p className="text-xs text-stone-500 font-medium">Letter Details</p>
          <p className="text-sm font-semibold text-stone-800 truncate">{letterData.subject || 'Untitled Letter'}</p>
          <p className="text-xs text-stone-400">Ref: {letterData.ref}</p>
          {isGeneratingPDF && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            >
              {loading || isGeneratingPDF ? 'Sending...' : 'Send Letter'}
            </GoldButton>
          </div>
        </form>
      </div>
...
interface SuperAdminLetterProps {
  isOpen: boolean;
  onClose: () => void;
  initialData?: Partial<AdminLetterData>;
  templateFile?: File | null;
}

const SuperAdminLetter: React.FC<SuperAdminLetterProps> = ({
  isOpen,
  onClose,
  initialData,
}) => {
  ...
  const [fromField, setFromField] = useState(initialData?.from || '');
  ...
  const buildLetterData = (): AdminLetterData => ({
    ref: refField,
    date: dateField,
    to: toField,
    from: fromField,
  ...
  const handleDownload = async (format: DownloadFormat) => {
    ...
    try {
      ...
      if (format === 'docx') {
        await generateAdminLetterDocx(letterData);
      } else if (format === 'pdf') {
        await generateAdminLetterPdf(letterData);
      }
      toast.success(`Letter downloaded as ${format.toUpperCase()}`);
    } catch (err) {
      console.error(`Failed to generate ${format} letter:`, err);
  ...
  const handleOpenSendModal = () => {
    if (!bodyText.trim()) {
      toast.error('Please add content to the letter before sending.');
      return;
    }
    setShowSendModal(true);
  };

  const handleSendLetter = async (recipientId: string, note: string) => {
    setIsSending(true);
    ...
    try {
      ...
      // Cast the result as Blob since we're passing true for returnBlob
      const pdfResult = await generateAdminLetterPdf(letterData, true);
      const pdfBlob = pdfResult as Blob;
      const pdfFile = new File([pdfBlob], `Letter_${letterData.ref || 'untitled'}.pdf`, { type: 'application/pdf' });

      // Create and send the letter via the API with the PDF attached
      await dispatch(createLetter({
        data: {
          to: letterData.to,
      ...
      onClose(); // Close the letter modal after sending
    } catch (err) {
        ...
        <div className="max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-xl bg-white shadow-2xl">
          <div className="flex items-center justify-between border-b border-stone-100 px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-[#1a3d1c]">Super Admin Letter</h3>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={handlePrint}
          ...
          <div className="max-h-[70vh] overflow-y-auto p-4 space-y-4">
            ...
            <div className="rounded-lg border border-stone-200 bg-stone-50 p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <label
                    htmlFor="letter-signature-upload"
              ...
              {currentUser?.signature_url ? (
                ...
                <div className="flex items-center gap-3 p-3 bg-white rounded border border-dashed border-stone-300">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-stone-400">Upload your signature to include it in the letter</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="border border-stone-300 bg-white p-6 shadow-sm font-sans text-black">
              ...
              <div className="text-center mb-4">
                <p className="text-sm font-bold uppercase tracking-wider text-stone-700">THE JUDICIARY</p>
                <p className="text-base font-bold uppercase tracking-wider text-stone-800">
              ...
              <div className="flex justify-end mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="w-48 bg-transparent border-0 border-b border-dashed border-transparent px-1 -mx-1 hover:border-stone-300 focus:border-stone-500 focus:outline-none text-sm"
                  />
                </div>
              </div>
              ...
              <div className="flex justify-end mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="w-40 bg-transparent border-0 border-b border-dashed border-transparent px-1 -mx-1 hover:border-stone-300 focus:border-stone-500 focus:outline-none text-sm"
                  />
                </div>
              </div>
              ...
              <div className="mb-6">
                <div className="flex items-center mb-2">
                  <span className="w-12 shrink-0 font-bold text-sm">To:</span>
                  <input
                    type="text"
                    value={toField}
                    onChange={(e) => setToField(e.target.value)}
                    placeholder="RECIPIENT NAME AND ADDRESS"
                    className="flex-1 bg-transparent border-0 border-b border-dashed border-transparent px-1 -mx-1 hover:border-stone-300 focus:border-stone-500 focus:outline-none text-sm uppercase"
                  />
                </div>
              </div>
              ...
              <div className="mb-6">
                <div className="flex items-center mb-2">
                  <span className="w-12 shrink-0 font-bold text-sm">From:</span>
                  <input
                    type="text"
                    value={fromField}
              ...
              <div className="mb-6">
                <div className="flex items-center mb-2">
                  <span className="w-12 shrink-0 font-bold text-sm">Subject:</span>
                  <input
                    type="text"
                    value={subjectField}
              ...
              <div className="mb-6">
                <textarea
                  ...
                  rows={14}
                  placeholder="Enter the letter content here..."
              ...
              <div className="mt-8">
                <p className="text-sm font-semibold text-stone-700">Yours sincerely,</p>
                <div className="mt-8 space-y-1">
                  ...
                  <div className="space-y-0.5">
                    <input
                      ...
                      className="block w-full bg-transparent border-0 border-b border-dashed border-transparent px-1 -mx-1 hover:border-stone-300 focus:border-stone-500 focus:outline-none text-sm font-bold uppercase"
                    />
...
export default SuperAdminLetter;