...
import {
  ...
  selectUsersMutating,
  selectIsSuperAdmin,
  selectUsersSignatureLoading,  // Fixed: changed from selectSignatureUploading
...
function ProfileTab() {
  ...
  const signatureLoading = useAppSelector(selectUsersSignatureLoading); // Fixed: renamed from selectSignatureUploading

  const [form, setForm] = useState({ full_name: '', email: '' });
  const fileInputRef = useRef<HTMLInputElement>(null);

  // Sync form when user data first loads
  ...
  return (
    <div className="space-y-4">
      ...
      <SectionCard
        ...
        description="Upload your signature image to be used when signing official documents."
        icon={<Image size={16} />}
      >
        <div className="space-y-4">
          {/* Current Signature Preview */}
          <div>
            <FieldLabel>Current Signature</FieldLabel>
            {user?.signature_url ? (
              <div className="flex items-center gap-4 rounded-lg border border-stone-200 bg-stone-50 p-4">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg border border-red-200 bg-white px-3 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50 hover:border-red-300 transition disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {signatureLoading ? <Loader2 size={14} className="animate-spin" /> : <Trash2 size={14} />}
                  Remove
                </button>
              </div>
            ) : (
              <div className="rounded-lg border border-dashed border-stone-300 bg-stone-50 p-6 text-center">
                <Image size={32} className="mx-auto text-stone-300" />
                <p className="mt-2 text-sm text-stone-500">No signature uploaded</p>
                <p className="text-xs text-stone-400">Upload a signature image to use in official documents</p>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <button
              ...
              disabled={signatureLoading}
              className="inline-flex items-center gap-2 rounded-lg border border-stone-200 bg-white px-4 py-2 text-sm font-medium text-stone-700 hover:bg-stone-50 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <p className="mt-1.5 text-[10px] text-stone-400">
              Accepted formats: JPEG, PNG, WEBP, GIF, SVG · Max size: 2MB
            </p>
          </div>
        </div>
      </SectionCard>
