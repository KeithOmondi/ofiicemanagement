...
const MemoandLetters: React.FC = () => {
  ...
  const canView = !!user;
  const isHelpdeskTab = activeTab === "helpdesk_approvals";

  // ─── Filter documents to include certificates ──────────────────────────────
  ...
  // ─── Fetch documents (only for non-helpdesk tabs) ──────────────────────────
  useEffect(() => {
    if (!canView || isHelpdeskTab) return;
  ...
  }, [dispatch, activeTab, searchQuery, canView, isHelpdeskTab]);
  ...
  const refreshDocuments = useCallback(() => {
    if (isHelpdeskTab) return;
    const params: DocumentFilters = { page: 1, limit: 10 };
    if (activeTab === "my_action") params.for_my_action = true;
    if (searchQuery) params.search = searchQuery;
    dispatch(fetchDocuments(params));
  }, [dispatch, activeTab, searchQuery, isHelpdeskTab]);
        ...
        {isSuperAdmin && (
          <button
            ...
            className={`rounded-md px-3 py-1.5 text-xs font-semibold transition-colors flex items-center gap-1 ${
              isHelpdeskTab ? "bg-[#1E4620] text-white" : "text-stone-500 hover:bg-stone-100"
            }`}
          >
            Helpdesk Approvals
          </button>
