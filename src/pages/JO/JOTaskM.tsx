...
import JOStandaloneTasks from "./JOStandaloneTasks";
import AnyDoDashboard from "../admin/SuperAdminToDo";
...
const EditTaskModal: React.FC<{
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const cleanInput: UpdateProjectTaskInput = {};

    if (formData.title && formData.title.trim()) {
      cleanInput.title = formData.title.trim();
    }
    if (formData.description !== undefined) {
      cleanInput.description = formData.description || null;
    }
    if (formData.status) {
      cleanInput.status = formData.status;
    }
    if (formData.priority) {
      cleanInput.priority = formData.priority;
    }
    if (formData.assignee !== undefined) {
      cleanInput.assignee = formData.assignee || null;
    }
    if (formData.deadline) {
      cleanInput.deadline = formData.deadline.includes("T")
        ? formData.deadline
        : new Date(`${formData.deadline}T00:00:00.000Z`).toISOString();
    } else {
      cleanInput.deadline = null;
    }
...
const ProjectsView: React.FC = () => {
  ...
  const handleEditProject = (input: UpdateProjectInput) => {
    if (editingProject) {
      ...
      if (input.tags !== undefined) {
        cleanInput.tags = Array.isArray(input.tags) ? input.tags : [];
      }
  ...
  const handleDeleteProject = (projectId: string, projectTitle: string) => {
    if (window.confirm(`Delete project "${projectTitle}" and all its tasks? This cannot be undone.`)) {
...
const TodoView: React.FC = () => {
  return <AnyDoDashboard />;
};

// ─── MAIN COMPONENT ──────────────────────────────────────────────────────────
const JOTaskM: React.FC = () => {
  const dispatch = useAppDispatch();
      ...
      {currentView === "todo" ? (
        ...
        <>
          ...
          <div className="transition-all duration-200">
            ...
            {currentView === "standalone" && <JOStandaloneTasks />}
          </div>
        </>
      )}
    </div>
