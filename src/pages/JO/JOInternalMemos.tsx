// src/pages/principalregistry/JOInternalMemos.tsx
...
import DRDocs from '../principalregistry/DRDocs';
...
// ─── JOInternalMemos Page ──────────────────────────────────────────────────

type TabType = 'list' | 'documents';

const JOInternalMemos: React.FC = () => {
  ...
  useEffect(() => {
    // Fetch sensitizations - backend filters by authenticated user
  ...
  const handleDelete = (id: string) => {
    if (window.confirm('Are you sure you want to delete this sensitization?')) {
      dispatch(deleteSensitization(id))
        .unwrap()
        .then(() => toast.success('Sensitization deleted successfully.'))
        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-xl font-semibold text-[#1a3d1c] flex items-center gap-2">
              ...
              My Sensitization Memos
            </h1>
            <p className="mt-0.5 text-sm text-stone-500">
              {activeTab === 'list' 
                ? `${sensitizations.length} memo${sensitizations.length !== 1 ? 's' : ''}`
                    ...
                    <tr>
                      <td colSpan={6} className="border border-stone-200 py-16 text-center text-sm text-stone-400">
                        ...
                        <button 
                          ...
                          className="mt-2 inline-flex items-center gap-1 text-[#c9a84c] hover:underline"
                        >
                          <Plus size={14} />
...
export default JOInternalMemos;