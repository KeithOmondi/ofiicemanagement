...
function SurveyList({
  ...
  const handleExportExcel = async (surveyId: string, title: string) => {
    try {
      const result = await dispatch(exportExcel(surveyId)).unwrap();
      // Create a download link
      const url = URL.createObjectURL(result);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${title.replace(/\s+/g, '-')}-responses.xlsx`;
  ...
  const handleExportWord = async (surveyId: string, title: string) => {
    try {
      const result = await dispatch(exportWord(surveyId)).unwrap();
      // Create a download link
      const url = URL.createObjectURL(result);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${title.replace(/\s+/g, '-')}-responses.docx`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
              ...
              <td className="py-3 px-4">
                <div className="flex items-center justify-end space-x-2">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 text-xs font-semibold text-emerald-800 bg-emerald-50 hover:bg-emerald-100 rounded border border-emerald-300 transition-colors cursor-pointer"
                  >
                    Excel
                  </button>
                  <button
                    type="button"
                    onClick={() => handleExportWord(survey.id, survey.title)}
                    className="px-2.5 py-1 text-xs font-semibold text-blue-800 bg-blue-50 hover:bg-blue-100 rounded border border-blue-300 transition-colors cursor-pointer"
                  >
                    Word
                  </button>
                  <button
