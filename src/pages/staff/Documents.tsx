        ...
        <div className="px-6 py-4 border-t border-stone-100 shrink-0">
          <form onSubmit={handleSubmit} className="flex gap-2">
            <textarea
              ...
              placeholder={canRespond ? "Write a response..." : "You cannot respond to this document"}
              disabled={!canRespond || isResponding}
              rows={2}
              className="flex-1 px-3 py-2 bg-stone-50 border border-stone-200 rounded-xl text-sm outline-none focus:border-[#1d3331] transition-colors resize-none disabled:opacity-50 disabled:cursor-not-allowed"
            />
            <button
              type="submit"
              disabled={!canRespond || isResponding || !note.trim()}
...
const Documents: React.FC = () => {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      await dispatch(createJoDocument({ input, file: uploadFile })).unwrap();
      toast.success(`Document ${uploadIsDraft ? 'saved as draft' : 'sent for review'} successfully`);
  ...
  const handleEditDraft = async (id: string) => {
    ...
    const input: UpdateJoDocumentInput = { title: editTitle };

    try {
      await dispatch(updateJoDocumentDraft({ id, input })).unwrap();
      toast.success('Document updated successfully');
      setIsEditModalOpen(false);
      setEditTitle('');
      dispatch(fetchJoDocuments(filters));
    } catch (err) {
      const errorMessage = typeof err === 'string' ? err : 'Failed to update document';
      toast.error(errorMessage);
    }
  };
  ...
  const handleReplaceFile = async (id: string) => {
    ...
    try {
      await dispatch(replaceJoDocumentFile({ id, file: replaceFile })).unwrap();
      toast.success('File replaced successfully');
      setIsReplaceFileModalOpen(false);
      setReplaceFile(null);
      dispatch(fetchJoDocuments(filters));
    } catch (err) {
      const errorMessage = typeof err === 'string' ? err : 'Failed to replace file';
      toast.error(errorMessage);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await dispatch(deleteJoDocumentDraft(id)).unwrap();
      toast.success('Document deleted');
      dispatch(fetchJoDocuments(filters));
    } catch (err) {
      const errorMessage = typeof err === 'string' ? err : 'Failed to delete document';
      toast.error(errorMessage);
    }
  };
...
const handleViewFile = (fileUrl: string | null) => {
  ...
  const stats = useMemo(() => ({
    ...
    drafts: documents.filter((d: JoDocument) => d.status === 'draft').length,
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-serif font-bold text-[#1d3331]">JO Documents</h1>
          <p className="text-sm text-stone-500 mt-1">Judicial Officer Document Management</p>
        </div>
        <button
          onClick={() => setIsUploadModalOpen(true)}
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-[#1d3331] text-white text-sm font-bold hover:bg-emerald-800 transition-colors"
        >
          <Plus size={18} />
          Upload Document
        </button>
      ...
      <div className="grid grid-cols-5 gap-3 mb-6">
        ...
        <div className="bg-white rounded-xl border border-stone-200 p-3 text-center">
          <p className="text-lg font-bold text-stone-600">{stats.drafts}</p>
      ...
      <div className="bg-white rounded-2xl border border-stone-200 p-4 shadow-sm mb-6">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 md:pb-0">
            ...
            {(['all', 'draft', 'pending_review', 'approved', 'rejected'] as StatusFilter[]).map((status) => (
              <button
                key={status}
        ...
        <div className="flex flex-wrap items-center gap-3 mt-3 pt-3 border-t border-stone-100">
          ...
          <button
            ...
            className="ml-auto px-3 py-1.5 rounded-lg border border-stone-200 text-xs font-medium text-stone-500 hover:bg-stone-50 transition-colors"
          >
            Clear Filters
          </button>
        </div>
      ...
      {documents.length === 0 ? (
        <div className="bg-white rounded-2xl border border-stone-200 p-12 text-center">
          ...
          <p className="text-sm text-stone-400 mt-1">
            {searchTerm ? 'Try adjusting your search' : 'Upload your first JO document'}
          </p>
        </div>
      ) : (
        <>
                ...
                <div key={doc.id} className="bg-white rounded-xl border border-stone-200 shadow-sm hover:shadow-md transition-shadow p-4">
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          {doc.assigned_to_name && (
                            <div className="mt-1 text-xs text-stone-500">
                              <span className="font-medium">Sent to:</span> {doc.assigned_to_name}
                            </div>
                          )}
                    ...
                    <div className="flex flex-wrap items-center gap-2 shrink-0">
                      ...
                      {/* Response Button - Always visible if user can respond */}
                      {canRespond && (
                        <button
                      ...
                      {canDelete && (
                        <button
                          ...
                          disabled={actionInProgress.deleting === doc.id}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-red-200 text-red-600 text-[10px] font-bold hover:bg-red-50 transition-colors disabled:opacity-50"
                        >
                          <Trash2 size={14} />
                          Delete
                        </button>
  ...
  >
    ...
    File
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-stone-100 flex items-center justify-between">
              <h3 className="text-base font-bold text-stone-900">Upload JO Document</h3>
              <button
            ...
            <form onSubmit={handleUpload} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-sm outline-none focus:border-[#1d3331] transition-colors file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-[#1d3331] file:text-white hover:file:bg-emerald-800"
                  required
                />
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-stone-400">
                  (Uncheck to send directly for review)
                </span>
              </div>
      ...
      {isEditModalOpen && selectedDocument && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-stone-100 flex items-center justify-between">
              <h3 className="text-base font-bold text-stone-900">Edit Draft Title</h3>
              <button
                onClick={() => { setIsEditModalOpen(false); setEditTitle(''); }}
                className="p-1.5 rounded-full hover:bg-stone-100 transition-colors text-stone-400"
              >
                <X size={18} />
              </button>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  onClick={() => handleEditDraft(selectedDocument.id)}
                  disabled={actionInProgress.updatingDraft === selectedDocument.id}
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#1d3331] text-white text-sm font-bold hover:bg-emerald-800 transition-colors disabled:opacity-50"
                >
                  {actionInProgress.updatingDraft === selectedDocument.id ? (
                    <Loader2 size={16} className="animate-spin" />
                  ) : (
                    <Edit size={16} />
                  )}
      ...
      {isReplaceFileModalOpen && selectedDocument && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-stone-100 flex items-center justify-between">
              <h3 className="text-base font-bold text-stone-900">Replace File</h3>
              <button
                onClick={() => { setIsReplaceFileModalOpen(false); setReplaceFile(null); }}
                className="p-1.5 rounded-full hover:bg-stone-100 transition-colors text-stone-400"
              >
                <X size={18} />
              </button>
            ...
            <div className="p-6 space-y-4">
              ...
              <p className="text-xs text-stone-400">Current file: {selectedDocument.original_name || 'No file'}</p>
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-stone-400 mb-1">
                  New File *
                </label>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  onClick={() => handleReplaceFile(selectedDocument.id)}
                  disabled={actionInProgress.replacingFile === selectedDocument.id || !replaceFile}
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#1d3331] text-white text-sm font-bold hover:bg-emerald-800 transition-colors disabled:opacity-50"
                >
                  {actionInProgress.replacingFile === selectedDocument.id ? (
                    <Loader2 size={16} className="animate-spin" />
                  ) : (
                    <Upload size={16} />
                  )}
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                {selectedDocument.assigned_to_name && (
                  <div>
                    ...
                    <p className="text-stone-700 font-medium">{selectedDocument.assigned_to_name}</p>
                  </div>
                )}
